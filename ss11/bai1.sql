...
END //
DELIMITER ;
-- Lịch khám appointment_id = 105 hiện đang có trạng thái Completed nhưng procedure cũ vẫn cho phép hủy.
SELECT * FROM Appointments WHERE appointment_id = 105;
CALL CancelAppointment(105);
-- status của lịch khám 105 bị đổi từ 'Completed' → 'Cancelled',
-- trong khi theo nghiệp vụ chỉ lịch 'Pending' mới được phép hủy.
-- 2. Giải thích vì sao lỗi xảy ra 
-- Stored Procedure hiện tại chỉ thực hiện UPDATE theo appointment_id mà không kiểm tra trạng thái lịch khám trước khi cập nhật.
-- Vì vậy, mọi lịch hẹn (kể cả Completed hoặc Cancelled) đều có thể bị đổi sang 'Cancelled', gây sai lệch dữ liệu nghiệp vụ.
...
CREATE PROCEDURE CancelAppointment(IN p_appointment_id INT)
BEGIN
    -- Chỉ hủy lịch khám đang ở trạng thái Pending
    UPDATE Appointments
    SET status = 'Cancelled'
    WHERE appointment_id = p_appointment_id
    AND status = 'Pending';
