...
END //
DELIMITER ;
-- Câu lệnh CALL tái hiện lỗi nhập số âm
-- Mã vật tư item_id = 10 hiện có tồn kho là 1000. Nhân viên nhập nhầm -50
SELECT * FROM Inventory WHERE item_id = 10;
CALL AddInventory(10, -50);
-- Giải thích nguyên nhân lỗi
-- nên khi p_quantity là số âm (-500), phép cộng biến thành trừ kho, làm thất thoát dữ liệu tồn kho thay vì báo lỗi.

-- Viết lệnh DROP để xóa thủ tục cũ.
DROP PROCEDURE IF EXISTS AddInventory;
-- Viết lệnh CREATE tạo lại thủ tục AddInventory mới. 
DELIMITER //
...
BEGIN
    -- Chỉ cộng kho nếu số lượng nhập hợp lệ (> 0)
