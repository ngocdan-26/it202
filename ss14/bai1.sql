...
-- Giả lập sự cố hệ thông bất ngờ xảy ra tại đây (Cúp điện, rớt mạng...)
SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Lôi: Hệ thống gặp sự cố mạng đột ngột!';
...
-- 2. Tạo Procedure mới có cơ chế bảo vệ Transaction
DELIMITER //
...
BEGIN
    ...
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Giao dịch thất bại: Đã hoàn tác mọi thay đổi!';
    END;
    START TRANSACTION;
    UPDATE Wallets 
