...
BEGIN
    ...
    BEGIN
        ROLLBACK;
        SELECT 'Thất bại: Lỗi hệ thống không xác định' AS Message;
    END;
    START TRANSACTION;
    ...
    ELSE
        ...
        ELSE
            ...
            COMMIT;
            SELECT 'Thành công: Đã xử lý đơn hàng' AS Message;
        END IF;
    END IF;
END //
DELIMITER ;
...
CALL ProcessEquipmentPurchase(1, 101, 2); 

CALL ProcessEquipmentPurchase(1, 101, 1000); 

CALL ProcessEquipmentPurchase(1, 102, 50); 

CALL ProcessEquipmentPurchase(2, 101, 1);