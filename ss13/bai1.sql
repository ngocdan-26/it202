...
FOR EACH ROW
BEGIN
-- Lôi logic: Đang lây ngày cũ ra so sánh với hiện tại thay vì kiểm tra ngày mới
	IF OLD.appointment_date < NOW() THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Lôi: Không thê đặt lịch khám vào thời điểm trong quá khứ!';
	END IF;
END //
DELIMITER ;
--  Viết câu lệnh UPDATE để tái hiện lỗi dời lịch.
UPDATE Appointments 
SET appointment_date = '2023-01-15 08:00:00' 
WHERE appointment_id = 104;
/*
Sự khác biệt: OLD.appointment_date là giá trị ngày khám trước khi cập nhật (thời điểm cũ), còn NEW.
appointment_date là giá trị ngày khám mới mà người dùng vừa nhập vào.
Lỗi logic: Đoạn mã cũ thường mắc lỗi khi so sánh NEW.appointment_date với 
OLD.appointment_date thay vì so sánh với thời gian thực tại (NOW()).
*/
-- viêt lại đoạn mã
-- 1. Xóa trigger cũ nếu tồn tại
DROP TRIGGER IF EXISTS PreventPastAppointments;

-- 2. Tạo trigger mới để chặn việc dời lịch về quá khứ
DELIMITER //
...
FOR EACH ROW
BEGIN
    -- Kiểm tra nếu ngày hẹn mới (NEW) nhỏ hơn thời gian hiện tại (NOW)
    IF NEW.appointment_date < NOW() THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Lỗi: Không được phép dời lịch khám về thời điểm trong quá khứ!';
    END IF;
END //
DELIMITER ;
