...
-- Do bảng chưa có INDEX trên cột full_name, MySQL sẽ phải quét toàn bộ bảng (Full Table Scan).
-- 1. Kiểm tra execution plan trước tối ưu
EXPLAIN
...
-- key = NULL
-- rows = quét toàn bộ bảng

-- 2. Tạo INDEX tối ưu tìm kiếm theo tên
CREATE INDEX idx_patient_name
ON patients(full_name);

-- 3. Kiểm tra execution plan sau tối ưu
EXPLAIN
SELECT patient_id, full_name, age, room_number
FROM patients
WHERE full_name = 'Tran Thi B';
...
-- rows: giảm đáng kể
-- Query sử dụng INDEX thay vì quét toàn bộ bảng
