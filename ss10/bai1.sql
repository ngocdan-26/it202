...
-- View đang hiển thị thêm 2 cột nhạy cảm:
-- hiv_status 
-- mental_health_history 
CREATE VIEW reception_patient_view AS
SELECT patient_id, full_name, age, room_number, hiv_status, mental_health_history 
FROM patients;
-- Viết lệnh UPDATE tuổi của một bệnh nhân thành -5 thông qua View để chứng minh hệ thống đang thiếu cơ chế kiểm soát đầu vào.
UPDATE reception_patient_view
SET age = -5
WHERE patient_id = 1;

-- Viết lệnh SQL xóa (DROP) View cũ.
DROP VIEW IF EXISTS reception_patient_view;

-- Viết lệnh CREATE tạo View mới khắc phục lỗ hổng
CREATE VIEW reception_patient_view AS
SELECT 
    patient_id,
    full_name, age,
