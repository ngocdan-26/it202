...
-- Triển khai: Viết mã SQL tạo View mang tên department_revenue_view bám sát giải pháp đã thiết kế.
...
GROUP BY 
    d.department_id,
    d.department_name;
-- Kiểm thử luồng chuẩn: Viết lệnh SELECT truy vấn trên View vừa tạo để hiển thị báo cáo.
SELECT * 
FROM department_revenue_view;

-- Kiểm thử bẫy dữ liệu: Viết một lệnh UPDATE cố tình thay đổi total_revenue của một khoa bất kỳ thông qua View, 
-- qua đó chứng minh thao tác này sẽ bị hệ thống chặn đứng
UPDATE department_revenue_view
SET total_revenue = 99999999
WHERE department_id = 1;
/*
View này không thể UPDATE được vì:
Có sử dụng hàm tập hợp (COUNT(), SUM())
Có GROUP BY
Là dữ liệu tổng hợp (aggregate view)
=> SQL tự động đánh dấu đây là non-updatable view (View chỉ đọc).
*/